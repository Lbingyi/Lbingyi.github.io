...
class UserFn extends controller
{
    ...
    {
		$getdb=['order_id'=>$orderid];
		$updb=['rech_time'=>time(),'sales_status'=>2];
		$type = Db::name('record')->where('id',$orderid)->value('typeid');
    	if($type < 1){
			/*
				添加功能;
			*/
			$bodydb=Db::name('acard')->where($getdb)->where('sales_status',1)->update($updb);
			return -1;

		}else{
			$data = Db::name('card')->where($getdb)->where('sales_status',1)->select();
			$num = count($data);
			$day = Db::name('card_type')->where('id',$data[0]['type'])->value('day');
			$dbuser = Db::name('user_app')->where('appid',$data[0]['appid'])->where('uid',$data[0]['uid'])->find();
			$updata['used_time'] = $dbuser['expire_time']>time()?$dbuser['expire_time']:time();
			$updata['new_time'] = $updata['used_time'] + ($num*$day*24*3600);
			$updata['day_num'] = $num*$day;
			$aa = ['expire_time'=>$updata['new_time']];
			Db::name('user_app')->where('uid',$data[0]['uid'])->where('appid',$data[0]['appid'])->update($aa);
			$bodydb=Db::name('card')->where($getdb)->where('sales_status',1)->update($updb);
			return $updata;
		}
		return 0;
    }
