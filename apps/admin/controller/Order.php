...
class Order extends Base {
    ...
    */
	public function lists(){
		...
		$total=Db::name('record')->where('pay_status!=0')->count();	//计算总数						       
        $orderdb=Db::name('record')
		->alias('a')
		->field('a.id,a.ordersn trade_no,d.name user,c.title sname,b.name,a.created date,a.amount money,a.pay_status')
        ->join('card_type b','b.id = a.typeid')
        ->join('goods c','c.goods_id = a.goods_id')
        ->join('member d','d.uid = a.user_id')
  ...
  //           $orderdb=Db::name('order')->where('user','like',$search.'%')->order('id desc')->page($offset.','.$limit)->select();
		...
		foreach($orderdb as $key=>$val){
            ...
            $trade_status=$orderdb[$key]['pay_status'];
