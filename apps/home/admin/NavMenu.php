		...
		if(input('get.type')=='pags'){
			...
			if($name){
				...
				foreach($name as $k=>$v){
					if($v=='home'){
						...
						$ars[]=['name'=>'应用商店','menu_url'=>$request->domain().URL('app/Goods/index')];
					}elseif($v=='allarticle'){
						$ars[]=['name'=>'所有文章','menu_url'=>$request->domain().URL('home/Article/allarticle')];
					}										
			...
			if($id){
				...
				foreach($id as $key=>$val){
					...
					$ars[]=['name'=>$sortdb['sort_name'],'menu_url'=>$request->domain().URL('home/Article/sorts',['id'=>$val])];
