...
class Score extends Base
{
    ...
    {
    	$this->redirect('home/Goods/index', ['cate_id' => 0]);
        $userdb=Db::name('member')->where('uid',session('user.user_id'))->find();
		...
		$this->assign('freevisa',$freevisa);
				$seo=[
		    'title'=>'支付 - '.config('global_config.site_name'),
    ...
    {
    	// header("Location:".url('home/Goods/index'));
    	$this->redirect('home/Goods/index', ['cate_id' => 0]);
    ...
    public function scorelist(){
		...
		$total=Db::name('order')->where('uid',session('user.user_id'))->count();	//计算总数						       
        $orderdb=Db::name('order')-> where('uid',session('user.user_id'))->order('id desc')->limit($offset.','.$limit)->select();
		if(input('?get.search')){
			$search=input('get.search');			
			$total=Db::name('order')->where('uid',session('user.user_id'))->where('name','like',$search.'%')->count();	//计算总页数						       
            $orderdb=Db::name('order')-> where('uid',session('user.user_id'))->where('name','like',$search.'%')->order('id desc')->limit($offset.','.$limit)->select();
