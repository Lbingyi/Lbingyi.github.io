    ...
    public function index()
    {
        ...
        if(request()->isAjax()){
			...
			if(!captcha_check(input('post.captcha'))){
                $res=['msg'=>'error','code'=>4007,'content'=>'验证码不正确'];
            	return json($res);
            }
			...
			if($dat){
	            ...
	            $safes=$ipsafe->safes(config('global_config.register_time'),config('global_config.register_num'),config('global_config.register_black_count'));
		        if($safes){
