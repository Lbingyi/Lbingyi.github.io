	...
	public function findpwd() {
		if(Request::instance()->isPost()){
			...
			if ($info) {
	            ...
	            $safes=$ipsafe->safes(config('global_config.email_repwb_time'),config('global_config.email_repwb_num'),config('access_control.email_repwb_black_count'));
		        if($safes){
				...
				// 生成URL			
			    $findpwd = URL('home/Respassword/editpwd','','',true) . '?key=' . $key . '&info=' . $string . '&time='.$time.'&code=' .$code; // code是用来检验time是否有修改过
			    $username = $info ['name'];
			    $title="加速哟网游加速器-重置密码";
			
			    $content="<h3>亲爱的：$username 用户</h3>
			    <br>您好!
			    <br>您已经进行了密码重置的操作，请点击以下链接(或者复制到您的浏览器)，以确认您的新密码重置操作！
			    <br>$findpwd 
			    <br><h4>有效期30分钟</h4>
			    <br>加速哟网游加速器-加速全球网游
			    <br>专注加速  全心服务
