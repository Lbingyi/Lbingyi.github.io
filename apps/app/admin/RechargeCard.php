...
class RechargeCard extends Base {
	...
	public function lists(){
		...
		if(input('?get.appid') && input('?get.status') && input('?get.sales_status') && input('?get.typeid')){
            ...
            $carddb=Db::name('card')->where($where)->order('id desc')->page($offset.','.$limit)->select();
		    if(input('?get.search')){
			    $search=input('get.search');			
			    $total=Db::name('card')->where($where)->where('card_number','like',$search.'%')->count();	//计算总页数						       
                $carddb=Db::name('card')->where($where)->order('id desc')->where('card_number','like',$search.'%')->page($offset.','.$limit)->select();			
		    }
		}else{
			$total=Db::name('card')->count();	//计算总数						       
            $carddb=Db::name('card')->order('id desc')->page($offset.','.$limit)->select();
		    if(input('?get.search')){
			    $search=input('get.search');			
			    $total=Db::name('card')->where('card_number','like',$search.'%')->count();	//计算总页数						       
                $carddb=Db::name('card')->order('id desc')->where('card_number','like',$search.'%')->page($offset.','.$limit)->select();			
		    }
		}			
