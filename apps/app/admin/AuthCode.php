...
class AuthCode extends Base {
	...
	public function lists(){
		...
		if(input('?get.appid') && input('?get.status') && input('?get.sales_status')){
            ...
            $acarddb=Db::name('acard')->where($where)->order('id desc')->page($offset.','.$limit)->select();
		    if(input('?get.search')){
			    $search=input('get.search');			
			    $total=Db::name('acard')->where($where)->where('acard_number','like','%'.$search.'%')->count();	//计算总页数						       
                $acarddb=Db::name('acard')->where($where)->order('id desc')->where('acard_number','like','%'.$search.'%')->page($offset.','.$limit)->select();			
		    }
		}else{
			$total=Db::name('acard')->count();	//计算总数								       
            $acarddb=Db::name('acard')->order('id desc')->page($offset.','.$limit)->select();
		    if(input('?get.search')){
			    $search=input('get.search');			
			    $total=Db::name('acard')->where('acard_number','like','%'.$search.'%')->count();	//计算总页数						       
                $acarddb=Db::name('acard')->order('id desc')->where('acard_number','like','%'.$search.'%')->page($offset.','.$limit)->select();			
		    }	
		}			
