...
class UserAcard extends Base
{
    ...
    public function acardlist(){
		...
		$total=Db::name('acard')->where('uid',session('user.user_id'))->count();	//计算总数						       
        $acarddb=Db::name('acard')-> where('uid',session('user.user_id'))->order('sales_time desc')->limit($offset.','.$limit)->select();
		if(input('?get.search')){
			$search=input('get.search');			
			$total=Db::name('acard')->where('uid',session('user.user_id'))->where('name','like',$search.'%')->count();	//计算总页数						       
            $acarddb=Db::name('acard')-> where('uid',session('user.user_id'))->where('name','like',$search.'%')->order('sales_time desc')->limit($offset.','.$limit)->select();
