...
class UserCard extends Base
{
    ...
    public function cardlist(){
		...
		$total=Db::name('card')->where('uid',session('user.user_id'))->count();	//计算总数						       
        $carddb=Db::name('card')-> where('uid',session('user.user_id'))->order('sales_time desc')->limit($offset.','.$limit)->select();
		if(input('?get.search')){
			$search=input('get.search');			
			$total=Db::name('card')->where('uid',session('user.user_id'))->where('name','like',$search.'%')->count();	//计算总页数						       
            $carddb=Db::name('card')-> where('uid',session('user.user_id'))->where('name','like',$search.'%')->order('sales_time desc')->limit($offset.','.$limit)->select();
